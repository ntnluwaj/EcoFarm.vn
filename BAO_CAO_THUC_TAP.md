...
```mermaid
graph TD
    A["Giai đoạn 1: Phân tích yêu cầu (Requirements)"] --> B["Giai đoạn 2: Thiết kế hệ thống (Design)"]
    B --> C["Giai đoạn 3: Lập trình (Implementation)"]
    C --> D["Giai đoạn 4: Kiểm thử (Verification)"]
    D --> E["Giai đoạn 5: Triển khai & Bảo trì (Maintenance)"]
```
...
graph LR
    ...
    subgraph FarmerUC [Phân hệ Khách hàng]
        ...
        F2(["Quản lý giỏ hàng & Đặt hàng"])
        F3(["Đặt hàng & Yêu cầu hóa đơn VAT"])
        F4(["Quét mã VietQR động"])
        F5(["Tra cứu vận đơn & Lịch sử"])
        F6(["Xem Cẩm nang nông nghiệp"])
        F7(["Quản lý tài khoản & Lưu địa chỉ"])
        F8(["Đánh giá & Hỏi đáp Q&A"])
...
erDiagram
    ...
    PRODUCTS ||--o{ PRODUCT_QUESTIONS : has_questions
